/**
 * Ek-4/ab Raporu
 * TESPİT VE TAHDİT ÇALIŞMALARINA ESAS OLAN
...
const ek4abExcel = async (req, res, next) => {
  try {
    ...
    // req.body: { bbhb_id, cks_id, secili_uye_idler: [...] }
    const {bbhb_id, cks_id, secili_uye_idler} = req.body;
    ...
    if (ayarlar?.teknik_ekipler?.length) {
      // En son ekibi veya ilçeye uygun ekibi bul
      ...
      let tumUyeler = ekip?.uyeler || [];

      // Eğer frontend'den seçili üye ID'leri geldiyse, onları filtrele
      if (secili_uye_idler?.length) {
        teknikEkip = tumUyeler
          .filter(u => secili_uye_idler.includes(String(u._id)))
          .sort((a,b) => kurumSira(a.kurum) - kurumSira(b.kurum));
      } else {
        // Muhtar ve bilirkişileri hariç tut (köy seçimi gerekiyor)
        teknikEkip = tumUyeler
          .filter(u => {
            const k = (u.kurum||'').toLowerCase();
            return !k.includes('muhtar') && !k.includes('bilirkişi') && !k.includes('aza');
          })
          .sort((a,b) => kurumSira(a.kurum) - kurumSira(b.kurum));
      }
    }
    ...
    ws.columns = [
      {width:5},{width:28},{width:9},{width:9},{width:9},{width:7},{width:8},{width:9},
    ...
    s1.value = 'TESPİT VE TAHDİT ÇALIŞMALARINA ESAS OLAN ÇİFTÇİ AİLE, GEÇİM KAYNAĞI VE HAYVAN VARLIĞI BİLDİRİM CETVELİ';
    ...
    let hc=9;
    KOLONLAR.forEach(k=>{
      if(k.gs>0){ const c=mc(5,hc,5,hc+k.gs-1); c.value=k.grup; sty(c,GRI,WH,true); }
      hc++;
    });
    KOLONLAR.forEach((k,i)=>{ const c=ws.getRow(6).getCell(9+i); c.value=k.kisa; sty(c,GRI,WH,true); });
    ...
    isletmeciler.forEach((ist,idx)=>{
      ...
      row.getCell(1).value=idx+1;           sty(row.getCell(1),null,null,false,'center');
      row.getCell(2).value=ist.sahip||'–';  sty(row.getCell(2),null,null,false,'left');
      row.getCell(3).value=yem>0?yem:null;  sty(row.getCell(3),null,null,false,'center');
      row.getCell(4).value=sebze>0?sebze:null; sty(row.getCell(4),null,null,false,'center');
      row.getCell(5).value=hub>0?hub:null;  sty(row.getCell(5),null,null,false,'center');
      row.getCell(6).value=(yem+sebze+hub)>0?'X':null; sty(row.getCell(6),null,null,false,'center');
      row.getCell(7).value=(ist.toplam_bbhb||0)>0?'X':null; sty(row.getCell(7),null,null,false,'center');
      ...
      KOLONLAR.forEach((kolon,ki)=>{
        ...
        row.getCell(9+ki).value=n>0?n:null; sty(row.getCell(9+ki),null,null,false,'center');
        if(n){hTot[kolon.key]+=n;tH+=n;}
      });

      // Toplam hayvan ve BBHB — bold değil
      row.getCell(8).value=tH>0?tH:null; sty(row.getCell(8),null,null,false,'center');
      const bv=ist.toplam_bbhb||0;
      row.getCell(NT).value=bv>0?+bv.toFixed(2):null;
      row.getCell(NT).numFmt='#,##0.00'; sty(row.getCell(NT),null,null,false,'right');

      tot.yem+=yem;tot.sebze+=sebze;tot.hub+=hub;tot.hayvan+=tH;tot.bbhb+=bv;
    });

    // ── TOPLAM SATIRI ─────────────────────────────────────
    const tr=7+isletmeciler.length; ws.getRow(tr).height=ROW_H;
    mc(tr,1,tr,2); ws.getCell(`A${tr}`).value='T O P L A M'; sty(ws.getCell(`A${tr}`),LG,null,true,'center');
    ws.getCell(`C${tr}`).value=tot.yem>0?+tot.yem.toFixed(3):null;   sty(ws.getCell(`C${tr}`),LG,null,false,'center');
    ws.getCell(`D${tr}`).value=tot.sebze>0?+tot.sebze.toFixed(3):null; sty(ws.getCell(`D${tr}`),LG,null,false,'center');
    ws.getCell(`E${tr}`).value=tot.hub>0?+tot.hub.toFixed(3):null;   sty(ws.getCell(`E${tr}`),LG,null,false,'center');
    ws.getCell(`F${tr}`).value=null; sty(ws.getCell(`F${tr}`),LG);
    ws.getCell(`G${tr}`).value=null; sty(ws.getCell(`G${tr}`),LG);
    // Toplam hayvan ve BBHB — bold değil
    ws.getCell(`H${tr}`).value=tot.hayvan||null; sty(ws.getCell(`H${tr}`),LG,null,false,'center');
    KOLONLAR.forEach((k,ki)=>{
      const v=hTot[k.key]||0;
      ws.getCell(`${col(9+ki)}${tr}`).value=v>0?v:null; sty(ws.getCell(`${col(9+ki)}${tr}`),LG,null,false,'center');
    });
    ws.getCell(`${col(NT)}${tr}`).value=+tot.bbhb.toFixed(2);
    ws.getCell(`${col(NT)}${tr}`).numFmt='#,##0.00'; sty(ws.getCell(`${col(NT)}${tr}`),LG,null,false,'right');

    // ── NOTLAR ────────────────────────────────────────────
    let nr=tr+1;
    mc(nr,1,nr,NT);
    ws.getCell(`A${nr}`).value=`Not: Yukarıdaki hayvan sayıları ve ekiliş alanı verileri ${yil} yılı ÇKS ve Türkvet kayıtlarından alınmıştır.`;
    ...
    if(teknikEkip.length>0){
      ...
      satirlar.forEach((sat,si)=>{
        ...
        sat.forEach((u,i)=>{
          ...
          // İmza çizgisi — kişinin üstünde (Ad satırının 1 üstü)
          // imza boşluğu: iR-1 (sadece ilk satır grubu için tarih buraya gelir)
          // İmza çizgisi boş bırakma — satır 3 sonrası (kurum+2)
        ...
        ws.getRow(iR+2).height=18;   // Kurum
        ws.getRow(iR+5).height=28;   // İmza
        iR+=7;
      });

      // Tarih — ilk imzacı satırının üstünde (en üst imza satırından önce)
      // Tarihi en sona koy ama son imza satırının üstüne değil altına
      mc(iR,1,iR,NT);
